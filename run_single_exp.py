...
from slim.utils.parallel import torch_multiprocessing_parallelize, fake_parallelize
...
def execute(path: str, dataset_name: str, slim_version: str, seed: int) -> None:
    ...
    slim(X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test, dataset_name=dataset_name,
            slim_version=slim_version, pop_size=pop_size, n_iter=n_iter, elitism=True, n_elites=n_elites, p_inflate=0.7, p_inflate_post=0.0, iter_post=60,
...
if __name__ == '__main__':
    ...
    _ = fake_parallelize(execute, parameters, num_workers=-2)
