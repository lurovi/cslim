...
def read_csv_data(path: str, dataset_name: str, idx: int) -> dict[str, tuple[torch.Tensor, torch.Tensor]]:
    d: pd.DataFrame = pd.read_csv(os.path.join(path, dataset_name, 'train' + str(idx) + '.csv'))
    y: np.ndarray = d['target'].to_numpy()
    d.drop('target', axis=1, inplace=True)
    X: np.ndarray = d.to_numpy()
    result: dict[str, tuple[torch.Tensor, torch.Tensor]] = {'train': (torch.from_numpy(X).float(), torch.from_numpy(y).float())}
    d = pd.read_csv(os.path.join(path, dataset_name, 'test' + str(idx) + '.csv'))
    y = d['target'].to_numpy()
    d.drop('target', axis=1, inplace=True)
    X = d.to_numpy()
    result['test'] = (torch.from_numpy(X).float(), torch.from_numpy(y).float())
    return result
